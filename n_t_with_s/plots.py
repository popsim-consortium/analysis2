...
def plot_all_ne_estimates(sp_infiles, msmc_infiles, outfile,
                          model, n_samp, generation_time, species, selection,
    ...
    plt.suptitle(f"{species}, population id {pop_id}, selection on {selection}", fontsize=16)
...
def plot_stairwayplot_coalrate(sp_infiles, outfile,
                               model, n_samp, generation_time, species, selection=None,
    ...
    ax.plot(steps, 1/(2*coal_rate), c="black")
    ax.set_title(f"stairwayplot with selection in {selection}")
